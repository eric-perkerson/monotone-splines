...
class IntegratedSplines:
    ...
    """
    def __init__(self, xs, ys, mode, num_knots, degree=4, support_min=None, support_max=None):
        """_summary_
    ...
    def predict(self, xs):
        ...
        """
        design_matrix = build_design_matrix(xs, self.num_knots, self.degree, self.knots_augmented)
    ...
    def show(self):
        ...
        design_matrix_plt = build_design_matrix(x_plt, self.num_knots, self.degree, self.knots_augmented)
...
def show_plots():
    """Generate plots to visualize spline basis functions for various degrees"""
...
def build_design_matrix(xs, m, k, knots_augmented, verbose=False):
    """Builds the design matrix Phi, where the ith column is the value of the ith basis spline computed on each x-value in xs
    ...
    xs : np.array
        x-values to predict on
    m : int
        Number of (interior) knots. The total number of augmented (interior + exterior) knots will be m + 2
    k : int
        Degree of the monotone spline. For integrated splines, the actual polynomial degree will be k - 1
    knots_augmented : np.array
...
def main():
    ...
    x_plt = np.linspace(0, x_max, num=num_points)
    y_plt = integrated_splines.predict(x_plt)
